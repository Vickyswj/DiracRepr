...
Definition ket0_n' (n : N) := kron_n n ket0.
...
Definition ket1_n' (n : N) := kron_n n ket1.

Hint Unfold ket0 ket1 : U_db.
Hint Unfold ketp ketn brap bran ket0_n ket1_n : S_db.


Lemma rrr :  N.to_nat 2=2%nat .
Proof. auto. Qed.

Lemma uu1 :   Npos (xO xH) = (N.of_nat (S (S O))).
Proof. auto. Qed.

Lemma uu2 :   Npos xH = (N.of_nat (S O)).
Proof. auto. Qed.

Ltac orthogonal_reduce' :=
autounfold with U_db;
replace (@M2MM 1 2) with (@M2MM (N.of_nat(N.to_nat 1)) (N.of_nat(N.to_nat 2))) by (repeat rewrite Nnat.N2Nat.id; auto);
replace  (@M2MM 2 1 (@MM2M 2 1 D.ket0))
with (@M2MM (N.of_nat 2) (N.of_nat 1) (@MM2M 2 1 D.ket0)) by (rewrite uu1,uu2; repeat rewrite idMM; auto);
replace  (@M2MM 2 1 (@MM2M 2 1 D.ket1))
with (@M2MM (N.of_nat 2) (N.of_nat 1) (@MM2M 2 1 D.ket1)) by (rewrite uu1,uu2; repeat rewrite idMM; auto);
repeat rewrite idMM;
rewrite rr,rrr;
try rewrite D.Mmult_bra0_ket0;
try rewrite D.Mmult_bra0_ket1;
try rewrite D.Mmult_bra1_ket0;
try rewrite D.Mmult_bra1_ket1;
auto.
