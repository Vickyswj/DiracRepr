...
Fixpoint Uf (n:nat) : Matrix (2 * 2 ^ (N.of_nat n)) (2 * 2 ^ (N.of_nat n))  :=
  match n with
  | O => I 2
  | S n' =>    (@Mmult _ _ (2 * 2 ^ (N.of_nat n)) (CX ⊗ I (2^ (N.of_nat n'))) (@Mmult _ _ (2 * 2 ^ (N.of_nat n))  (I 2 ⊗ (Uf n')) (CX ⊗ I (2^ (N.of_nat n')))))
end.
(*which argument*)
...
Lemma DJ_1 :
(n > 0)%nat ->
@Mmult _ _ (1 ^ (N.of_nat n) * 1)  (Uf n) ((kron_n n ∣+⟩) ⊗ ∣-⟩) ≡ ((kron_n n  ∣+⟩) ⊗ ∣-⟩).
Proof.
Opaque N.mul.
...
- simpl.
   ...
   repeat rewrite p2.
   rewrite <- (N.mul_comm 1 (1 * (1 * 1 ^ N.of_nat n0))).
   rewrite (N.mul_assoc 1 1 (1 * 1 ^ N.of_nat n0)).
   rewrite (N.mul_assoc 2 2 (2 * 2 ^ N.of_nat n0)).
   rewrite Mmult_assoc.
   rewrite Mmult_assoc.
   ...
   rewrite (kron_assoc _ (kron_n n0 ∣+⟩) ∣-⟩).
   ...
   rewrite kron_assoc.
   rewrite (N.mul_assoc 1 1 (1^ N.of_nat n0)).
    rewrite (N.mul_assoc 2 2 (2^ N.of_nat n0)).
    repeat rewrite <- p3.
    rewrite (N.mul_comm (2 * 2 * 2 ^ N.of_nat n0) 2).
    rewrite (N.mul_comm (1 * 1 * 1 ^ N.of_nat n0) 1).
   rewrite kron_mixed_product.
   ...
   rewrite (N.mul_comm (1^ N.of_nat n0) ) in *.
   rewrite IHn0.
   rewrite <- I2_eq.
   rewrite Mmult_Ipos.
   rewrite kron_assoc.
   ...
   rewrite <- (kron_assoc ∣+⟩ ∣+⟩ _).
   rewrite (N.mul_comm 2 (2^ N.of_nat n0)).
   rewrite kron_mixed_product.
   rewrite Mmult_1_l.
   rewrite CXpp.
   reflexivity. lia.
